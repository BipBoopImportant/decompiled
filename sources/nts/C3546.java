package nts;

/* renamed from: nts.ȡ  reason: contains not printable characters */
public class C3546 extends C3599 {

    /* renamed from: ᖑ  reason: contains not printable characters */
    public byte[] f38 = null;

    public C3546() {
        this.f3542 = 49;
    }

    /* renamed from: ϴ  reason: contains not printable characters */
    public int m103(byte[] bArr, int i10) {
        int i11 = 0;
        int r32 = super.m3270(bArr, i10);
        if (this.f3538 && this.f38 == null) {
            byte b10 = bArr[i10 + 1];
            if ((b10 & 128) > 0) {
                this.f38 = new byte[((b10 & Byte.MAX_VALUE) + 2)];
                while (true) {
                    byte[] bArr2 = this.f38;
                    if (i11 >= bArr2.length) {
                        break;
                    }
                    bArr2[i11] = bArr[i10 + i11];
                    i11++;
                }
            } else {
                this.f38 = new byte[]{bArr[i10], b10};
            }
        }
        return r32;
    }

    /* renamed from: ഇ  reason: contains not printable characters */
    public void m104(C3796 r42) {
        C3796 r02 = new C3796();
        int i10 = 0;
        while (i10 < this.f385.size()) {
            if (this.f385.get(i10) instanceof C3981) {
                ((C3981) this.f385.get(i10)).m2352(r02);
                i10++;
            } else {
                throw new C3625((Throwable) null, 222, C3727.m1052("@lfr0gsuykqvt;awthr!km,^k{GoK@G_UEEWe]QKZK\u0017\u0001-'3q&2>48>*075z 65)3`*,m\u001f*:\u0006.\n>\u0001\u0006\u001e\u0014\u0004\u0004\u0016$\u001c\u0010\n\u001b\nV0\u001c\u0016\u0002@\u0017\u0003\u000f\u0005\t\u000f\u001b\u0001\u0006\u0004K\u0011\u0007\u0004\u0018\u0002Q\u001b\u001d\\/:1+\u001c\u000b\u000e!+%\"nÂîäð²åñý÷ûýéóôö¹ãõöêð£éï®ÝÈÃÙîùüÓÙ×Ð ª¾ü«¿³¹µ³§½º¸÷­»¸¤¾í§¡à ·²Ò«ÕÑÊÆµ§×Ì°ÎÊÑÝ®¼Ì×L`j~<ksyusg}zx7m{xd~-ga SFMW`wr]WY^\u0012+\u0006\u0007\u001b\u001e\b\u001d\u001c\u0005\u0005&\u0002\u0010\u0004%\b\u0016\r\u001f\u0015\bS\u001f\u000f\u0000\u0014\u001c\u0017\\\\V\u001e;i$$8m=:01-10 \"i;\u0001\u001e\b\t\u0004\u000f\u0007\u000f\u001d\u0001\u0001B \u000f\u000f\n\u001a\u0012\tW/\u0001\t\u0013W\u001b\u0017\u0018\u0016\u0013\u0005n&(($?!//\"6eÚàÿéèåîæîüàà£òôâó÷þ¼ðÿà¸ñùçùòæ¿¾ÇØÜÕÜª¦®§½§¹¤®øáäÄÆÌÀÊÖËÅÅÏËÌÙÞËÉÚÎØÀÀÔÕÞÒÜïï¨àà¯ÉõöæîåããÇàèïñÿ÷üýé÷ÜâÒóåàüÂLvi~sxpxjvv5UDX)|rzh,iatng\"lj'CAL^TYY[}TBG[S]TW]MdXnI]ZHFJADPB\bJflx:myusua{|~1k}~bx+ag&B|qgmddZ|YONR^PUV@PAr^UmJ^YCMAJGSAV\fÃïåñ³äðüöúüèòõ÷¸âô÷ëñ¢èî¯ÊäëîùýÉÛÑÚÆÚÆÙÝöÚØÒÏÓßÑÂÔÖ¼¼¤ñ¥¢¤¥µ©¬¼ºÿª¸°°©®¨çª°§©­»èÄÎÚÏÛ×ÝÑ×ÃÙÞÜÉßÜÀÚÃÅöÏÀÎÄÐêòûñì¶Txrf$sgkamkeb`/uc`|f5y8\\boysmvOOQ\rYuk)~jfl`frhom\"xnmqk8rt5Qobt~`{BB\\\u00005zxb1cfbmsmj||mJ^W_XLxZCzT_V\ne*(2a362=#=:,,etBA]G\u0014SSZW_SS[\u0011×íòäåèãëãñíí®Ìããæöþå»Ãíåÿ»÷ûôúÿéÊÄÄÈÓÍÃÃÎÚ¢¨¼þ©½±»·±¥¿¸ºõ¯¹º¦¼ï¥£â¸µ¢½Ð", 46, 46, 85));
            }
        }
        r02.f1337 = this.f3542;
        r42.m1336(r02.m1349());
    }

    /* renamed from: ᐳ  reason: contains not printable characters */
    public C4114 m105() {
        return new C3981();
    }

    /* renamed from: ⶊ  reason: contains not printable characters */
    public int m106(byte[] bArr, int i10) {
        this.f3541 = C3823.m1630(bArr, i10, this.f3539);
        return super.m384(bArr, i10);
    }
}
